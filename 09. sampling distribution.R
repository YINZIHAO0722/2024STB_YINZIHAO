#데미터에서 크기 2인 표본추출
salary=c(45,50,50,55,55,60, 60,65,70,90) #면 봉데이터
com2=combn(salary, 2) #면봉데미터에서 2개씩 선택해서 데미터로 생성, com2에 저장
print (com2)

#표본의 크기2로 추출된 데이터의 평균을 구한 뒤 막대그래프 작성
Comt=t(com2)#데미터의 전치치
z=rowMeans(Comt)#전치된 데미터의 행평균 계산
barplot(table(z),xlab="연봉평균",ylab="도수",main="연봉평균의 분포")

# 표본평균의 기댓값 구하기
meanofz = mean(z) # z의 평균
print(meanofz)# 표본평균의 기댓값 구하기
...
print(meanofz2)
barplot(table(z2), xlab="평균2", ylab="도수", main="표본 크기2의 도수분포표")
...
print(meanofz3)
barplot(table(z3), xlab="평균3", ylab="도수", main="표본 크기3의 도수분포표")
...
print(meanofz5)
barplot(table(z5), xlab="평균3", ylab="도수", main="표본 크기5의 도수분포표")
...
# iris 데이터 사용하여 표본추출, Species를 기준으로
sample2 = getdata(data = iris, m = sam2) # 추출된 데이터 저장
print(sample2)# 표본 추출방법 : 층화추출법
install.packages("sampling")
library(sampling)